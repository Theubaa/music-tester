...
function decodeFile(arrayBuffer) {
    audioCtx.resume().then(() => {
        audioCtx.decodeAudioData(arrayBuffer).then(async function handleDecodedAudio(audioBuffer) {
            ...
            if (essentia) {
                try {
                    console.log('Computing BPM/Key with Essentia.js...');
                    essentiaAnalysis = computeKeyBPM(prepocessedAudio);
                    console.log('BPM/Key computation successful:', essentiaAnalysis);
                } catch (error) {
                    console.warn('Error computing BPM/Key:', error);
                    essentiaAnalysis = null;
                }
            } else {
                console.warn('Essentia.js not loaded yet');
                essentiaAnalysis = null;
            }
...
function collectPredictions() {
    if (inferenceResultPromises.length == modelNames.length) {
        Promise.all(inferenceResultPromises).then((predictions) => {
            ...
            // Only update BPM/Key if essentiaAnalysis is available
            console.log('Checking essentiaAnalysis for BPM/Key update:', essentiaAnalysis);
            if (essentiaAnalysis && typeof essentiaAnalysis === 'object' && essentiaAnalysis.bpm && essentiaAnalysis.keyData) {
                console.log('Updating BPM/Key with data:', essentiaAnalysis);
                resultsViz.updateValueBoxes(essentiaAnalysis);
            } else {
                console.log('Skipping BPM/Key update - essentiaAnalysis is not available:', essentiaAnalysis);
                // Don't call updateValueBoxes at all - let it show default values
            }
