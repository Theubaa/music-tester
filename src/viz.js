class AnalysisResults {
    ...
    updateValueBoxes(essentiaAnalysis) {
        try {
            ...
            if (!this.bpmBox || !this.keyBox) {
                console.warn('BPM/Key DOM elements not found');
                return;
            }
            ...
            // Extra safety check - if essentiaAnalysis is undefined/null, just return
            if (!essentiaAnalysis) {
                console.warn('Essentia analysis is undefined/null:', essentiaAnalysis);
                return;
            }
            
            if (typeof essentiaAnalysis !== 'object' || !essentiaAnalysis.bpm || !essentiaAnalysis.keyData) {
                console.warn('Essentia analysis data not available:', essentiaAnalysis);
                return;
            }
            ...
            console.error('Error updating BPM/Key display:', error);
            if (this.bpmBox) this.bpmBox.textContent = 'N/A';
            if (this.keyBox) this.keyBox.textContent = 'N/A';
        }
    }
}
